...
impl Database {
    pub async fn read_owners(&self) -> Result<Vec<Owner>> {
        sqlx::query(r#"SELECT * FROM `owners`"#)
            .fetch_all(&self.0)
            .await?
            .iter()
            .map(Owner::from_row)
    ...
    pub async fn create_owner(&self, o: Owner) -> Result<()> {
        sqlx::query(r#"INSERT INTO `owners` (`id`, `name`, `group`) VALUES (?, ?, ?)"#)
            .bind(o.id)
            .bind(o.name)
            .bind(o.group)
            .execute(&self.0)
    ...
    pub async fn update_owner(&self, id: &str, o: Owner) -> Result<()> {
        sqlx::query(r#"UPDATE `owners` SET `id` = ?, `name` = ?, `group` = ? WHERE `id` = ?"#)
            .bind(o.id)
            .bind(o.name)
            .bind(o.group)
            .bind(id)
    ...
    pub async fn delete_owner(&self, id: &str) -> Result<()> {
        sqlx::query(r#"DELETE FROM `owners` WHERE `id` = ?"#)
            .bind(id)
            .execute(&self.0)
            .await?;
        Ok(())
